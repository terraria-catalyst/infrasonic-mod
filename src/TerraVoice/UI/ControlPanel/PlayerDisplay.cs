...
internal class PlayerDisplay : SmartUIElement
{
    ...
    private void DrawCurrentPlayer(SpriteBatch spriteBatch, Rectangle drawBox)
    {
        ...
        {
            DrawMidpointString(spriteBatch, Language.GetTextValue($"Mods.TerraVoice.UI.NotOnline"), drawBox);
        ...
        if (onlinePlayers <= 1)
        {
            DrawMidpointString(spriteBatch, Language.GetTextValue("Mods.TerraVoice.UI.NoOtherPlayers"), drawBox);
        ...
        spriteBatch.DrawString(TerraVoice.Font, $"{player.name}", drawPosition, TerraVoice.Cyan);
        ...
        spriteBatch.DrawString(TerraVoice.Font,
            Language.GetTextValue($"Mods.TerraVoice.UI.Muted{(playerMuted ? "Yes" : "No")}"),
            drawPosition,
            playerMuted ? TerraVoice.Cyan : TerraVoice.Pink);
    }
    ...
    private void DrawMidpointString(SpriteBatch spriteBatch, string display, Rectangle drawBox)
    {
        ...
        Vector2 halfSize = TerraVoice.Font.MeasureString(display).RoundEven() / 2;

        spriteBatch.DrawString(TerraVoice.Font, display, middle - halfSize, TerraVoice.Cyan);
    }
